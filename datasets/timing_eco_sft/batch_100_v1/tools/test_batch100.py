...
class EvidenceParserTests(unittest.TestCase):
    def test_observable_path_evidence_is_checkpoint_only_and_replay_stable(self) -> None:
        report = """###############################################################
#  Generated by:      Cadence Innovus 21.10-p004_1
#  Design:            NV_NVDLA_CMAC_CORE_mac
###############################################################
Path 1: VIOLATED Setup Check with Pin u/capture/CK
Endpoint:   u/capture/D    (^) checked with leading edge of 'clk'
Beginpoint: u/launch/Q (^) triggered by leading edge of 'clk'
Analysis View: functional_setup_ss
= Slack Time                   -0.085
     Timing Path:
     +-----------------------------------------------+
     | Pin | Edge | Net | Cell | Delay | Arrival | Required |
     | u/clock/Y | ^ | clk1 | BUF_X16M_A9TR40 | 0.100 | 0.1 | 0.2 |
     | u/launch/Q | ^ | n0 | DFFQ_X1M_A9TR40 | 0.300 | 0.4 | 0.5 |
     | u/slow/A | ^ | n0 | NAND2_X0P5M_A9TR40 | 0.004 | 0.4 | 0.5 |
     | u/slow/Y | v | n1 | NAND2_X0P5M_A9TR40 | 0.842 | 1.2 | 1.3 |
     | u/other/A | v | n1 | INV_X1M_A9TR40 | 0.003 | 1.2 | 1.3 |
     | u/other/Y | ^ | n2 | INV_X1M_A9TR40 | 0.210 | 1.4 | 1.5 |
     | u/capture/D -> | ^ | n2 | DFFQ_X1M_A9TR40 | 0.000 | 1.4 | 1.5 |
     Other End Path:
"""
        with tempfile.TemporaryDirectory(prefix="b100-instruction-report-") as temporary:
            path = Path(temporary) / "target_setup_before.rpt"
            path.write_text(report)
            observed = evidence.observable_target_path_evidence(path)
        self.assertEqual(observed["design"], "NV_NVDLA_CMAC_CORE_mac")
        self.assertEqual(observed["analysis_view"], "functional_setup_ss")
        path_evidence = observed["paths"]["u/capture/D"]
        self.assertEqual(path_evidence["beginpoint"], "u/launch/Q")
        self.assertEqual(
            [row["instance"] for row in path_evidence["cells"]],
            ["u/slow", "u/other"],
        )
        self.assertEqual(path_evidence["cells"][0]["current_ref"], "NAND2_X0P5M_A9TR40")
        evidence.compare_observable_path_evidence(observed, copy.deepcopy(observed))

    def test_instruction_renderer_is_evidence_complete_and_oracle_safe(self) -> None:
        observed = {
            "setup_wns_ns": -0.085,
            "setup_tns_ns": -0.085,
            "hold_wns_ns": 0.050,
            "hold_tns_ns": 0.0,
            "negative_endpoints": ["u/capture/D"],
            "target_paths": {
                "u/capture/D": {
                    "cell_delay_ns": 7.5,
                    "net_delay_ns": 0.02,
            ...
            "drc_count": 6954,
            "connectivity_violations": 6,
        }
        path_evidence = {
            "design": "NV_NVDLA_CMAC_CORE_mac",
            "analysis_view": "functional_setup_ss",
            "paths": {
                "u/capture/D": {
                    "beginpoint": "u/launch/Q",
                    "cells": [
                        {
                            "instance": "u/slow",
                            "pin": "u/slow/Y",
                            "current_ref": "NAND2_X0P5M_A9TR40",
                            "delay_ns": 0.842,
                        }
                    ],
                }
        ...
        text = instruction.render_instruction(
            case_id="B1_CASE_002",
            expected_modification_count=1,
            observable=observed,
            path_evidence=path_evidence,
        )
        instruction.assert_oracle_safe(text)
        self.assertIn("beginpoint=u/launch/Q", text)
        self.assertIn("inst=u/slow", text)
        self.assertIn("current_ref=NAND2_X0P5M_A9TR40", text)
        self.assertIn("setEcoMode -batchMode true", text)
        self.assertIn("setup WNS>=0、TNS=0", text)
        self.assertNotIn("NAND2_X2M_A9TR40", text)

    def test_instruction_oracle_markers_are_rejected(self) -> None:
        for marker in (
            "hidden oracle",
            "injection_operations",
            "baseline_ref=NAND2_X2M_A9TR40",
            "直接逆操作",
            "注入前尺寸",
        ):
            with self.subTest(marker=marker), self.assertRaises(BatchError):
                instruction.assert_oracle_safe(f"请使用 {marker}。")

    def test_system_prompt_matches_tcl_allowlist(self) -> None:
        self.assertIn("setEcoMode -batchMode true/false", instruction.SYSTEM_PROMPT)
        self.assertIn("ecoChangeCell", instruction.SYSTEM_PROMPT)
        self.assertIn("refinePlace -eco true", instruction.SYSTEM_PROMPT)

    def test_conversation_omits_construction_metadata(self) -> None:
        with tempfile.TemporaryDirectory(prefix="b100-conversation-") as temporary:
            case_root = Path(temporary)
            (case_root / "instruction.txt").write_text(
                "请依据 violating checkpoint 的可观察证据完成 ECO。\n"
            )
            (case_root / "answer.txt").write_text(
                "```tcl\nrefinePlace -eco true\n```\n"
            )
            conversation = finalize_batch._conversation(
                case_root,
                {
                    "id": "B1_CASE_001",
                    "shape": "one_endpoint_one_cell",
                    "strategy": "endpoint_local_rvt_downsize",
                    "difficulty": "medium",
                },
                {
                    "fix_sha256": "1" * 64,
                    "violating_checkpoint_sha256": "2" * 64,
                    "violating_checkpoint_bundle_sha256": "3" * 64,
                    "violating_checkpoint_bundle_hash_algorithm": "test.v1",
                },
            )
        self.assertNotIn("shape", conversation["metadata"])
        self.assertNotIn("strategy", conversation["metadata"])
        self.assertNotIn("endpoint_local_rvt_downsize", repr(conversation))
    ...
    def test_execute_one_runs_the_two_fresh_replays_concurrently(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="b100-execute-test-") as temporary:
            ...
            with (
                ...
                patch.object(
                    batch100.evidence,
                    "observable_target_path_evidence",
                    return_value={
                        "design": "top",
                        "analysis_view": "setup",
                        "paths": {
                ...
                patch.object(batch100.evidence, "compare_observable_path_evidence"),
                patch.object(
                    batch100.instruction_text,
                    "render_instruction",
                    return_value="请修复可观察的 setup 违例。\n",
                ),
