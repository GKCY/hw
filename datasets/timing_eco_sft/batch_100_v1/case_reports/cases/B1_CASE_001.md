...
```text
案例 B1_CASE_001 的 violating checkpoint 在 exp 层级观察到 setup WNS=-0.061000 ns、TNS=-0.061000 ns。负裕量 endpoint 恰为：u_exp/exp_sft_09_reg_1_/D。目标 data path 的 cell/net delay (ns) 为：u_exp/exp_sft_09_reg_1_/D=7.383900/0.272000。DRV 计数：transition=0，capacitance=0，fanout=431。请给出严格 1 个不同实例的最小 RVT 组合逻辑 resize ECO，并在末尾仅执行 refinePlace -eco true；不得修改连接、约束、时钟或路由。
```
